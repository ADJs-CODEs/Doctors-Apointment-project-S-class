        ...
        {appointments.map((item, index) => (
          <div className='flex flex-wrap justify-between max-sm:gap-2 sm:grid sm:grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50' key={item._id}>
            ...
            <div className='flex items-center gap-2'>
              <img className='w-8 rounded-full' src={item.userData?.image} alt={item.userData?.name || "Patient"} /><p>{item.userData?.name}</p>
            </div>
            <p className='max-sm:hidden'>{calculateAge(item.userData?.dob)}</p>
            <p>{slotDateFormat(item.slotDate)}, {item.slotTime}</p>
            <div className='flex items-center gap-2'>
              <img className='w-8 rounded-full' src={item.docData?.image} alt={item.docData?.name || "Doctor"} /><p>{item.docData?.name}</p>
            </div>
            ...
            {item.cancelled
              ...
              : item.isCompleted
                ...
                : <img onClick={() => cancelAppointment(item._id)} className='w-10 cursor-pointer mx-auto hover:opacity-70 transition-all' src={assets.cancel_icon} alt="" />}
