...
const Dashboard: React.FC = () => {
  ...
  const { aToken, cancelAppointment, dashData, getDashData } = useContext(
    AdminContext,
  ) as AdminContextType;
  const { slotDateFormat, setProgress } = useContext(
    AppContext,
  ) as AppContextType;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 md:mb-10">
          ...
          <p className="text-slate-500 text-xs sm:text-sm font-medium mt-1">
            Administrative control panel for Serene Medical.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 md:mb-12">
          ...
          <div
            ...
            className="bg-white p-6 sm:p-8 rounded-[32px] sm:rounded-[40px] shadow-portal border border-slate-100 flex items-center gap-6 group hover:bg-slate-900 transition-all duration-500 cursor-pointer active:scale-95"
          >
            ...
            <div>
              <p className="text-xl sm:text-2xl font-black text-slate-900 group-hover:text-white leading-none">
                {dashData.doctors}
              </p>
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[2px] text-slate-400 mt-2 group-hover:text-teal-400">
                Specialists
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[2px] text-slate-400 mt-2 group-hover:text-indigo-400">
                Bookings
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-[32px] sm:rounded-[40px] shadow-portal border border-slate-100 flex items-center gap-6 group hover:bg-slate-900 transition-all duration-500 cursor-pointer active:scale-95 sm:col-span-2 lg:col-span-1">
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[2px] text-slate-400 mt-2 group-hover:text-rose-400">
                Patients
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[32px] sm:rounded-[48px] shadow-portal border border-slate-100 overflow-hidden">
          <div className="px-6 sm:px-10 py-6 sm:py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
            <div className="flex items-center gap-3">
              ...
              <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[2px] sm:tracking-[3px] text-slate-900">
                Live Activity Feed
              </p>
            </div>
            <button
          ...
          <div className="divide-y divide-slate-50">
            {dashData.latestAppointments.map((item: Appointment) => (
              ...
              >
                <img
                  className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl object-cover shadow-sm border-2 border-white"
                  src={item.docData.image}
                  alt=""
                />

                <div className="flex-1 text-center sm:text-left">
                  <p className="text-sm font-black text-slate-900 group-hover:text-teal-600 transition-colors">
                    {item.docData.name}
                  </p>
