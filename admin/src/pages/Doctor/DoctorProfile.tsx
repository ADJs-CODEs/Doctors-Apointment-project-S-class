...
const DoctorProfile: React.FC = () => {
  const { profileData, getProfileData } = useContext(
    DoctorContext,
  ...
  useEffect(() => {
    if (profileData && !isEdit) setTempData(profileData);
  }, [profileData, isEdit]);
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 bg-slate-50/50 min-h-screen animate-reveal">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row gap-6 md:gap-8 bg-white p-6 sm:p-8 md:p-10 rounded-[40px] md:rounded-[40px] shadow-sm border border-slate-100">
          <div className="w-full md:w-64 flex justify-center md:block">
            <img
              className="bg-primary/10 w-48 h-48 sm:w-64 sm:h-64 md:w-full md:h-auto aspect-square object-cover rounded-3xl shadow-inner"
              src={displayData.image.replace(
                "/upload/",
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-slate-500 font-bold text-sm sm:text-base">
              {displayData.degree} — {displayData.speciality}
            </p>
            ...
            {/* ✅ FIXED: Correct props passed to Availabilty */}
            <Availabilty
              isEdit={isEdit}
              tempData={tempData}
              setTempData={
                setTempData as React.Dispatch<
                  React.SetStateAction<{ available: boolean } | null>
                >
              }
              ...
              {isEdit ? (
                <>
                  <button
                    ...
                    className="w-full sm:w-auto bg-teal-600 text-white px-8 py-3.5 rounded-2xl font-black uppercase text-[10px] tracking-[2px] shadow-lg shadow-teal-500/20 active:scale-95 transition-all"
                  >
                    Save Changes
                  </button>
                  <button
                    ...
                    className="w-full sm:w-auto bg-slate-200 text-slate-600 px-8 py-3.5 rounded-2xl font-black uppercase text-[10px] tracking-[2px] active:scale-95 transition-all"
                  >
                    Cancel
                  </button>
                </>
                ...
                <button
                  ...
                  className="w-full sm:w-auto bg-slate-900 text-white px-10 py-3.5 rounded-2xl font-black uppercase text-[10px] tracking-[2px] shadow-lg shadow-slate-900/20 active:scale-95 transition-all"
                >
                  Edit Profile
                </button>
              )}
