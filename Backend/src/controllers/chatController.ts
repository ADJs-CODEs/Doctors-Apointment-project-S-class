...
const chatWithGeminiDoctor = async (req: Request, res: Response) => {
  try {
    ...
    if (appointmentContext) {
      const meds =
        appointmentContext.healthData?.prescribedMedicines
          ?.map(
            (m: any) =>
              `${m.name} (${m.dosagePerDay}x/day, ${m.remainingQuantity} left, adherence: ${Math.round(((m.totalQuantity - m.remainingQuantity) / m.totalQuantity) * 100)}%)`,
          )
          .join(", ") || "None";
