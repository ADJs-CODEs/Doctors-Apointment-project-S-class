...
const TopDoctors: React.FC = () => {
  ...
  return (
    <div className='flex flex-col items-center gap-6 my-12 md:my-24 px-4 md:mx-10'>
      ...
      <div className='text-center space-y-3 mb-6 md:mb-8'>
        <div className='inline-flex items-center gap-2 px-3 py-1 bg-teal-50 rounded-full text-teal-600'>
          ...
          <span className='text-[9px] md:text-[10px] font-black uppercase tracking-widest'>Elite Practitioners</span>
        </div>
        ...
        <p className='w-full sm:w-2/3 mx-auto text-slate-500 text-xs sm:text-sm font-medium px-2'>
          Access our most requested medical experts across the Lagos network.
        </p>
      </div>

      {/* --- Doctors Grid --- */}
      {/* Changed grid-cols-auto to a more predictable responsive flow */}
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 pt-5'>
        {doctors.slice(0, 10).map((item: Doctor, index: number) => (
          <motion.div
            ...
            className='medical-card group cursor-pointer border border-slate-100 hover:border-teal-500/30 transition-all duration-500 rounded-[24px] md:rounded-[32px] overflow-hidden bg-white shadow-sm hover:shadow-clinical'
          >
            {/* Image Container */}
      ...
      <motion.button
        ...
        className='mt-10 md:mt-16 w-full sm:w-auto flex items-center justify-center gap-3 bg-white border border-slate-100 px-8 md:px-10 py-3.5 md:py-4 rounded-xl md:rounded-2xl text-slate-900 font-black uppercase tracking-widest text-[10px] md:text-xs shadow-clinical hover:bg-slate-900 hover:text-white transition-all group'
      >
