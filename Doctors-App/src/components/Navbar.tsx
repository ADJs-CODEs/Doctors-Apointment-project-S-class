        ...
        <AnimatePresence>
          {showMenu && (
            <motion.div
              ...
              className='fixed inset-0 z-[60] bg-white md:hidden flex flex-col overflow-y-auto'
            >
              {/* Mobile Header */}
              <div className='flex items-center justify-between px-8 py-8 border-b border-slate-50'>
                ...
                <div onClick={() => setShowMenu(false)} className='p-2.5 bg-slate-50 rounded-xl cursor-pointer'>
                ...
                {['HOME', 'DOCTORS', 'ABOUT', 'CONTACT'].map((item, i) => (
                  <motion.div key={item} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <NavLink onClick={() => setShowMenu(false)} to={item === 'HOME' ? '/' : `/${item.toLowerCase()}`}>
                      <li className='text-4xl font-black tracking-tighter text-slate-900 hover:text-teal-500 transition-all uppercase'>
                        {item}
                      </li>
                    </NavLink>
                  </motion.div>
              ...
              {token && userData ? (
                <div className='mt-10 px-10 pt-10 border-t border-slate-100 flex flex-col gap-6'>
                  <p className='text-[10px] font-black text-slate-400 uppercase tracking-[3px]'>Account Management</p>
                  
                  <div onClick={() => {navigate('/my-profile'); setShowMenu(false)}} className='flex items-center gap-4 group'>
                    <div className='p-3 bg-teal-50 rounded-2xl text-teal-600'><UserGear size={28} weight="duotone" /></div>
                    <span className='text-xl font-bold text-slate-700'>My Profile</span>
                  </div>

                  <div onClick={() => {navigate('/my-appointments'); setShowMenu(false)}} className='flex items-center gap-4 group'>
                    <div className='p-3 bg-teal-50 rounded-2xl text-teal-600'><CalendarCheck size={28} weight="duotone" /></div>
                    <span className='text-xl font-bold text-slate-700'>Appointments</span>
                  </div>

                  <button onClick={logout} className='mt-4 flex items-center justify-center gap-3 bg-rose-50 text-rose-600 p-5 rounded-[24px] font-black uppercase tracking-widest text-xs'>
                    <SignOut size={20} weight="bold" /> Sign Out
                  </button>
                </div>
              ) : (
                <div className='mt-12 px-10'>
                  <button onClick={() => {navigate('/login'); setShowMenu(false)}} className='w-full bg-slate-900 text-white p-6 rounded-[24px] font-black uppercase tracking-widest text-xs shadow-xl shadow-slate-200'>
              ...
              {/* Mobile Footer */}
              <div className='mt-auto p-10 bg-slate-50/50'>
                <p className='text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2'>Health Hotline</p>
                <p className='text-2xl font-bold text-slate-900'>(+234) 704 203 0981</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
