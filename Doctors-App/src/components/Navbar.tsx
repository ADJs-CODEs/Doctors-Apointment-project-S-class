    ...
    <nav className='sticky top-0 z-50 py-4 px-6 md:px-10 bg-white/70 backdrop-blur-xl border-b border-slate-100 md:mt-4 md:mx-4 md:rounded-[28px] shadow-sm'>
      ...
      <div className='max-w-7xl mx-auto flex items-center justify-between'>
        ...
        <div className='flex items-center gap-6'>
          ...
          {/* BURGER ICON (Mobile Menu Trigger) */}
          <div onClick={() => setShowMenu(true)} className='md:hidden p-2.5 bg-slate-50 rounded-xl border border-slate-100 cursor-pointer active:scale-95 transition-transform'>
            <List size={24} weight="bold" className="text-slate-900" />
          </div>
        </div>

        {/* MOBILE MENU OVERLAY (Slide-in Logic) */}
        <AnimatePresence>
          {showMenu && (
            <>
              <motion.div
                  ...
                  <div className='mt-12 pt-8 border-t border-slate-100'>
                    {token && userData ? (
                      <div className='flex flex-col gap-5'>
                        ...
                        <div onClick={() => { navigate('/my-profile'); setShowMenu(false) }} className='flex items-center gap-4 py-2 cursor-pointer active:opacity-60 transition-opacity'>
                          <UserGear size={28} weight="duotone" className="text-slate-400" />
                          <span className='text-lg font-bold text-slate-700'>My Profile</span>
                        </div>
                        <div onClick={() => { navigate('/my-appointments'); setShowMenu(false) }} className='flex items-center gap-4 py-2 cursor-pointer active:opacity-60 transition-opacity'>
                        ...
                        <button onClick={logout} className='mt-4 flex items-center justify-center gap-3 bg-rose-50 text-rose-600 p-4 rounded-2xl font-black uppercase tracking-widest text-xs active:scale-95 transition-transform'>
                          <SignOut size={20} weight="bold" /> Sign Out
                        </button>
                      </div>
                    ) : (
                      <button onClick={() => { navigate('/login'); setShowMenu(false) }} className='w-full bg-slate-900 text-white p-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg active:scale-95 transition-transform'>
