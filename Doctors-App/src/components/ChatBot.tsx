...
const ChatBot: React.FC = () => {
  const { token, userData } = useContext(AppContext) as AppContextType;
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([
        {
          role: "assistant",
          content: `Hi ${userData?.name?.split(" ")[0] || "there"}! I'm your ADJ's CODEs health assistant. I can help you with your appointments, medications, vitals, and general health questions. What would you like to know?`,
        },
      ]);
    }
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      const { data } = await axiosInstance.post(API_PATHS.CHAT.MESSAGE, {
        message: messageText,
      ...
      <AnimatePresence>
        {!open && (
          <motion.button
            ...
            className="fixed bottom-6 left-6 z-50 w-14 h-14 bg-slate-900 text-white rounded-full shadow-2xl flex items-center justify-center border border-white/10 group"
          >
            <RiRobot2Line
              size={24}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-6 left-6 z-50 w-[340px] sm:w-[380px] h-[560px] bg-white border border-slate-100 rounded-[28px] shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
              {messages.map((msg, i) => (
                ...
                >
                  ...
                  <div
                    className={`max-w-[78%] px-3.5 py-2.5 rounded-2xl text-sm font-medium leading-relaxed ${
                      msg.role === "assistant"
