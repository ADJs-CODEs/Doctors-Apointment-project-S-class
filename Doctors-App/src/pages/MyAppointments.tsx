...
const MyAppointments: React.FC = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='max-w-5xl mx-auto px-4 py-12 bg-slate-50 min-h-screen relative'>
      ...
      <div className='mb-6'>
        <h1 className='text-2xl font-black text-slate-900 uppercase mb-6'>My Appointments</h1>
        
        {/* --- SEARCH BAR --- */}
        <div className='relative group'>
          <RiSearchLine className='absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-teal-500 transition-colors' size={20} />
          <input 
            type="text"
            placeholder="Search by doctor or specialty..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className='w-full bg-white border border-slate-200 rounded-2xl py-4 pl-12 pr-12 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all shadow-sm'
          />
          {searchQuery && (
            <button 
      ...
      <div className='space-y-4'>
        {loading ? (
                  ...
                  {isExpanded && item.healthData && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className='overflow-hidden'>
                      <div className={`rounded-[24px] md:rounded-[32px] p-6 text-white mt-1 ${isCritical ? 'bg-gradient-to-br from-red-950 via-slate-900 to-black' : 'bg-slate-900'}`}>
                        {/* Vitals & Meds content remains the same... */}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='text-center py-20'>
            <p className='text-slate-400 font-bold text-sm uppercase tracking-widest'>No appointments found for "{searchQuery}"</p>
          </motion.div>
        )}
      </div>
    </motion.div>
